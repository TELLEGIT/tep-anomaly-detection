...
- Fault 1 발생 시 반응기 압력(XMEAS_7)이 정상 구간 대비 급격히 변동하였다.
- Fault 4 발생 시 냉각수 계통 이상으로 인해 반응기 온도(XMEAS_9)가 상승하였다.
- SHAP 분석 결과 반응기 압력, 반응기 온도, 생성물 조성 관련 변수가 이상 판단에 가장 큰 영향을 미쳤다.
- 단순 이상 여부 탐지를 넘어 어떤 공정 변수가 이상의 원인이 되었는지 설명 가능한 형태로 분석하였다.
